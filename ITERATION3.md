...
| **M7: Comments + Agent primitives (user-requested, supersedes review flow)** | REMOVE the AI enrich review screen (Ctrl+E review UI from M3) — user verdict: "trash, buggy". Keep underlying llm/enrich machinery (prompt building, parsing, async jobs) as foundation for agents. CLI `enrich` subcommand may stay (non-interactive, no review UI). Task comments: db-backed (author, body, created_at), GitHub-like scrollable thread in card detail view, add-comment key. Agent primitives: user-defined agents via markdown config files (opencode-style: frontmatter with name/type/prompt, e.g. .kanban/agents/*.md + ~/.kanban/agents/*.md). Two agent TYPES: (1) COMMENT agents (output posted as comment, authored by agent); (2) DESCRIPTION agents (output updates task title/description). Agents receive task+board+project context. Trigger: NO dedicated agent UI — user adds comment; if it starts with @agentname, agent runs (async llm job) with its default prompt + user message; output applied per type. | Review screen gone. Comment thread in detail view. @agentname triggers in comments. |
