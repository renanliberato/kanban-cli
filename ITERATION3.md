...
| **M6: Background jobs polish + multiple boards** | Real LLM jobs with spinner indicators on cards. `-b` flag, `.kanban/` directory, `list-boards`. NOTE: AI Q&A scope moved to M7 (comment agents supersede it). | Submit an enrich job from TUI → spinner appears on card → result arrives → card updates live. `kanban -b project2` opens separate board. |
