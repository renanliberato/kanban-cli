...
| **M3: Non-interactive CLI + AI enrich** | Subcommand parser in `main.c` (`add`, `enrich`, `list`, `show`). Real `opencode run` provider. Enrich flow: add card with `--ai` flag → LLM expands → human review screen → confirm. Card model extended with description + labels. | `kanban add "fix login bug" --ai` → LLM proposes description + labels → review prompt → card saved with full metadata. Verify via `kanban show <id>`. |
