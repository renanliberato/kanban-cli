...
| **Status M5: IN-PROGRESS** | Single-column narrow mode. Tab between columns at <60 cols. Undo ring buffer (20 ops). Soft archive (`x` key) + visibility toggle. | Resize terminal to 40×24 → single column with tabs visible. Delete card → undo → card restored. Archive card → toggle visibility. |
