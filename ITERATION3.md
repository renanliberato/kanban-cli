...
| **Status M5: DONE** | Single-column narrow mode. Tab between columns at <60 cols. Undo ring buffer (20 ops). Soft archive (`x` key) + visibility toggle. 287 unit tests (85+56+61+57+28), 74 BDD scenarios (19 features), zero warnings. | Resize terminal to 40×24 → single column with tabs visible. Delete card → undo → card restored. Archive card → toggle visibility. |
| **Status M6: DONE** | Background jobs polish + multiple boards: card-level spinner/job-indicator on board view, 'K' cancels running job on selected card, "job queue full" flash when cap (3) hit, failure reason shown in flash (timeout/cancelled/non-zero exit), default timeout 120s (KANBAN_LLM_TIMEOUT override). -b/--board flag for named boards in TUI/CLI, `.kanban/` discovery (walk up cwd, stop at HOME), board_path_resolve with tested precedence matrix, `list-boards` CLI, board name in status bar. 329 unit tests (22 new board_path), 74 BDD scenarios green. Build zero warnings. | Submit a job from TUI → spinner on card while running → 'K' cancels → "job cancelled" flash. `kanban -b project` opens separate ~/.kanban/project.db. Board name shown in status bar. |
| **M7a: DONE** | Removed the enrich review screen. Ctrl+E now: submits enrich job → on completion applies result directly (overwrites description + merges labels, undo snapshot first), flashes "Enriched (u to undo)". Schema migration v2 adds `comments` table (id, card_id, author, body, created_at) with FK ON DELETE CASCADE and index on card_id. Detail view shows COMMENTS section (GitHub-like: author · date line dimmed, wrapped body) scrolling as one unit with description. `c` in detail view opens single-line input on status bar for new comment (ESC cancels, Enter confirms). Author from $USER env, fallback "me". CLI: `kanban comment <id> "text"` prints card id; `kanban show <id>` prints comment thread. Delete cascade verified. Undo does NOT restore deleted comments (documented limitation). BDD: 20 features / 76 scenarios. Unit: 381 asserts. Zero warnings. | Review screen gone. Ctrl+E enriches directly. Comment thread in detail view. `kanban comment` CLI works. |
| **M7b: DONE** | Agent primitives: user-defined agents via markdown config files. Two agent types: COMMENT agents and DESCRIPTION agents. @agentname triggers in comments. | Agent config in .md files with YAML-like frontmatter (name, type, prompt body). Discovery from ./.kanban/agents/ and ~/.kanban/agents/. @mention in comment text triggers async LLM job (TUI) or sync job (CLI). Comment agents post results as @authored comments; description agents update title/description. Unknown @mentions flash error. Recursion guard: agent-authored comments (author starts with '@') never re-trigger. `kanban agents` lists discovered agents. 443 unit tests (62 new agent tests), 21 BDD features (79 scenarios, 441 steps). Zero build warnings. |
